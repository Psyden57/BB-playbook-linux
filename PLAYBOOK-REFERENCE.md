...
- **WDT2 @0x4A314000** (wdtkick RE): kick = **complement WTGR @+0x30** each tick (never writes WCRR). Exit path only *reads* WSPR @+0x48 and prints "WDT is disabled now" if WSPR != 0x4444 — **nothing in userland ever disables WDT2**. Live read: WSPR=0x4444 ⇒ RUNNING (TI OMAP4 enable seq 0xBBBB→0x4444; **disable = 0xDDDD→0x0000**, each write followed by WWPS@+0x34 pending-bit clear). The kexec payload must service or disable WDT2 itself.
